...
class RepeatVisitInsertAttributes(BaseModel):
    ...
    primary_visit_id: int = Field(..., description="Идентификатор первичного посещения", gt=0)
    ...
    def validate_ids(cls, values):
        ...
        if "primary_visit_id" not in keys or values.get("primary_visit_id") is None:
            raise HTTPException(status_code=400, detail="Не указан идентификатор первичного посещения")
...
class RepeatVisitUpdateAttributes(BaseModel):
    ...
    primary_visit_id: Optional[int] = Field(None, description="Идентификатор первичного посещения")
